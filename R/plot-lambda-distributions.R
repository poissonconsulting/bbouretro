...
#'
#' Create histograms of simulated \eqn{\lambda} values for a chosen population
#' unit.
#'
#' @param lambda A data frame generated by `bbr_lambda()`.
#'
...
#' }
bbr_plot_lambda_distributions <- function(lambda) {
  chk::chk_data(lambda)
  chk_population(lambda)
  ...
  xmax <- ifelse(xmax <= max(lambda$estimate), max(lambda$estimate) + 0.05, xmax)
  xmin <- ifelse(xmin >= min(lambda$estimate), min(lambda$estimate) - 0.05, xmin)
  ...
  ggplot(LrawR, aes(.data$RanLambda)) +
    ...
    geom_vline(data = lambda, aes(xintercept = .data$estimate), color = "red") +

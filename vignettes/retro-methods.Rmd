...
vignette: >
  %\VignetteIndexEntry{Classical Methods for Estimating Survival, Recruitment, and Population Growth}
...
The classical or **retro** methods used in bbou**retro** are outlined below. 
...
Further details on the Kaplan-Meier approach as applied in boreal caribou studies are given in `vignette("methods")`.
...
A full summary of methods is given in `vignette("methods")`.

## Population Growth, $\lambda$

For `bbr_lambda()` function uses the basic Hatter-Bergerud method [@hatter_moose_1991].
...
Users can generate plots of simulated lambda values using the `bbr_plot_lambda_distributions()` or yearly estimates using `bbr_plot_lambda()`.

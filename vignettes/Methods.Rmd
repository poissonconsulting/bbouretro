...
The staggered entry Kaplan-Meier, which uses discrete time steps, is defined algebraically below (Pollock et al. 1989). 
...
Standard error is estimated using either the (Cox and Oakes 1984) uses Greenwood’s formula EQN XXX 
...
Further details on the Kaplan Meir approach as applied in boreal caribou studies are given in Pearson et al (2022).

For `recruitment()`

Estimation of recruitment follows methods of (DeCesare et al. 2012).
From Pearson et al (2022) we summarize the following equations: 
...
The basic Hatter-Bergerud equation used to estimate lamba is given below. 
